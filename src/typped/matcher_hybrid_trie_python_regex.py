...
complicated patterns and a `RegexTrieDict` for simple patterns.  It splits
the patterns into simple patterns for the trie and leaves the complex patterns
for the Python regexes.  Both matchers are run against the text, and the
best result of the two is returned.

This has good insert and delete time.  Matches for individual patterns should
be reasonably fast, but it is implemented in pure Python so it will not match
the Python regexes.  For many fairly simple patterns (fixed strings and strings
with wildcards) this method can be an improvement.

TODO: describe the algorithm and complexity.  Do after describing in that
matcher.
...
from .matcher_trie_regex import MatcherTrieRegex
from .matcher_python_regex import MatcherPythonRegex

# TODO below is just copied-over from the Python regex matcher except for
# the name.  Fill in the methods with the hybrid stuff.
...
class MatcherHybridTriePythonRegex(object):
    ...
    def __init__(self):
        ...
        self.python_matcher = MatcherPythonRegex() # NOTE
        self.trie_matcher = MatcherTrieRegex() # NOTE
