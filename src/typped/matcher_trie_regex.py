...
A matcher for a `TokenTable` that uses a `RegexTrieDict` to hold the
patterns.

TODO: does it use the RegexTrieDictScanner or the dict itself?
...
from .regex_trie_dict_scanner import RegexTrieDictScanner # TODO use this???
...
# TODO: below is just copied-over from Python regex version except for the
# name.  Replace the relevant methods.

class MatcherTrieRegex(object):
